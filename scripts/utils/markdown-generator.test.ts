/**
 * [INPUT]: 依赖 markdown-generator 的十四语言发布接口与 Muse Video 数据契约
...
const categories: FilterCategory[] = [
  ...
  {
    id: 61,
    title: "Camera Language & Immersive POV",
    slug: "camera-language-immersive-pov",
    parentId: 4,
...
test("renders a Muse Video empty-state README without inherited model copy", () => {
  const markdown = generateMarkdown(
    { all: [], featured: [], regular: [], stats: { total: 0, featured: 0 }, categories },
    0
  );

  assert.match(markdown, /Awesome Muse Video Prompts & Skills/);
...
test("groups a video prompt by its single production workflow", () => {
  ...
  assert.equal(groups[0].category.slug, "camera-language-immersive-pov");
