/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地视频提示词指纹与 Muse Video 模型证据
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  const hasPromptProvenance =
    /(?:tweet_text|reply_text|thread_reply|quoted_tweet|alt_text)_prompt|rewritten|normalized|official|reconstruction|completed/.test(
      promptSource
    );
  ...
  if (
    prompt.sourceMeta?.model_evidence !== "muse-video-official-preview"
  ) {
    hardFailures.push("missing verified Muse Video model evidence");
  }
  ...
  } else if (/(rewritten|normalized|official|reconstruction|completed)/.test(promptSource)) {
    score += 15;
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  if (!hasMuseVideoEvidence) {
    reasons.push("missing explicit Muse Video evidence; manual review required");
