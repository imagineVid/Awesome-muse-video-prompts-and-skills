/**
 * [INPUT]: 依赖 prompt-quality 的 Muse Video 评分、硬门槛与去重接口
...
test("accepts a fully attributed Muse Video video record", () => {
  const result = scoreStoredTwitterPrompt({
    ...
    sourceMeta: {
      ...
      model_evidence: "muse-video-official-preview",
    },
    videoCategories: { workflows: [{ slug: "native-audio-dialogue-foley" }] },
  });

  assert.equal(result.decision, "accept");
});

test("accepts a detailed Muse Video candidate with video evidence", () => {
...
test("rejects a launch announcement without a reusable shot brief", () => {
  const result = scoreTwitterCandidate({
    id: "announcement",
    text: "Muse Video is now available. Try it today!",
