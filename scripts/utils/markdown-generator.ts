 ...
 * [POS]: scripts/utils 的核心展示层，把 Muse Video 结构化数据渲染为 GitHub 原生页面
...
function generateHeader(locale: string): string {
  const coverImage = "public/images/muse-video-prompt-skills-cover.png";

  return `<a href="${REPO_URL}">
  <img src="${coverImage}" alt="Muse Video Prompt Skills" width="100%" />
</a>
...
**[${t("browseGallery", locale)}](${getMuseVideoProductUrl(locale)})**
...
function generatePromptSection(
  ...
  md += `**[${getPromptCtaLabel(locale)}](${getMuseVideoProductUrl(locale)})**\n\n`;
  md += "---\n\n";
...
function officialCaseCopy(locale: string): { section: string; note: string; caseLabel: string; promptLabel: string } {
  const copy: Record<string, { section: string; note: string; caseLabel: string; promptLabel: string }> = {
    en: { section: "Official Preview Cases", note: "Playable first-party preview clips. Editorial headings are localized; official source prompts remain in English for verification.", caseLabel: "Case", promptLabel: "Official source prompt (English)" },
    es: { section: "Casos oficiales de avance", note: "Clips reproducibles del avance oficial. Los encabezados están localizados y los prompts oficiales se conservan en inglés para verificarlos.", caseLabel: "Caso", promptLabel: "Prompt oficial de origen (inglés)" },
    pt: { section: "Casos oficiais de antevisão", note: "Clips reproduzíveis da antevisão oficial. Os títulos são localizados e os prompts oficiais mantêm-se em inglês para verificação.", caseLabel: "Caso", promptLabel: "Prompt oficial de origem (inglês)" },
    it: { section: "Casi ufficiali in anteprima", note: "Clip riproducibili dell'anteprima ufficiale. I titoli sono localizzati e i prompt ufficiali restano in inglese per la verifica.", caseLabel: "Caso", promptLabel: "Prompt ufficiale di origine (inglese)" },
    de: { section: "Offizielle Vorschaufälle", note: "Abspielbare Clips aus der offiziellen Vorschau. Überschriften sind lokalisiert; offizielle Quellprompts bleiben zur Prüfung auf Englisch.", caseLabel: "Fall", promptLabel: "Offizieller Quellprompt (Englisch)" },
    fr: { section: "Cas officiels en avant-première", note: "Clips lisibles de l'aperçu officiel. Les titres sont localisés; les prompts officiels restent en anglais pour vérification.", caseLabel: "Cas", promptLabel: "Prompt officiel source (anglais)" },
    ar: { section: "حالات المعاينة الرسمية", note: "مقاطع قابلة للتشغيل من المعاينة الرسمية. تُترجم العناوين وتبقى المطالبات الرسمية بالإنجليزية للتحقق.", caseLabel: "الحالة", promptLabel: "المطالبة الرسمية الأصلية (بالإنجليزية)" },
    ja: { section: "公式プレビュー事例", note: "公式プレビューの再生可能な映像です。見出しは翻訳し、公式プロンプトは検証用に英語のまま保持します。", caseLabel: "事例", promptLabel: "公式ソースプロンプト（英語）" },
    ko: { section: "공식 미리보기 사례", note: "공식 미리보기의 재생 가능한 영상입니다. 제목은 번역하고 공식 프롬프트는 검증을 위해 영어 원문을 유지합니다.", caseLabel: "사례", promptLabel: "공식 원문 프롬프트(영어)" },
    zh: { section: "官方预览案例", note: "来自官方预览的可播放片段。编辑标题已本地化，官方来源提示词保留英文以便核验。", caseLabel: "案例", promptLabel: "官方来源提示词（英文）" },
    nl: { section: "Officiële previewcases", note: "Afspeelbare clips uit de officiële preview. Koppen zijn vertaald; officiële bronprompts blijven Engels voor verificatie.", caseLabel: "Case", promptLabel: "Officiële bronprompt (Engels)" },
    ru: { section: "Официальные примеры из превью", note: "Воспроизводимые клипы из официального превью. Заголовки локализованы, официальные промпты сохранены на английском для проверки.", caseLabel: "Пример", promptLabel: "Официальный исходный промпт (английский)" },
    tr: { section: "Resmî ön izleme örnekleri", note: "Resmî ön izlemeden oynatılabilir klipler. Başlıklar çevrilir; resmî kaynak promptları doğrulama için İngilizce tutulur.", caseLabel: "Örnek", promptLabel: "Resmî kaynak promptu (İngilizce)" },
    pl: { section: "Oficjalne przypadki z zapowiedzi", note: "Odtwarzalne klipy z oficjalnej zapowiedzi. Nagłówki są lokalizowane, a oficjalne prompty pozostają po angielsku do weryfikacji.", caseLabel: "Przypadek", promptLabel: "Oficjalny prompt źródłowy (angielski)" },
  };
...
${t("learnMore", locale)} [Meta AI engineering overview](${META_MUSE_RELEASE_URL}) · [Meta product announcement](${META_AI_URL}) · [Create videos on ImagineVid](${getMuseVideoProductUrl(locale)})
...
**[${t("viewInGallery", locale)}](${getMuseVideoProductUrl(locale)})** •
