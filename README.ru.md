<a href="https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills">
  <img src="public/images/muse-video-prompt-skills-cover.png" alt="Muse Video Prompt Skills" width="100%" />
</a>

> Проверяемая по источникам библиотека описаний кадров, паттернов движения и аудиовизуальных рабочих процессов для Muse Video.
# Awesome Muse Video: промпты и навыки

[![Awesome](https://awesome.re/badge.svg)](https://github.com/sindresorhus/awesome)
[![GitHub stars](https://img.shields.io/github/stars/imagineVid/Awesome-muse-video-prompts-and-skills?style=social)](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills)
[![License: CC BY 4.0](https://img.shields.io/badge/License-CC%20BY%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![Update README](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills/actions/workflows/update-readme.yml/badge.svg)](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills/actions)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](docs/CONTRIBUTING.md)

> Изучайте бриф, смотрите результат, находите автора и переносите логику режиссуры, а не копируйте внешнюю стилистику.

> **Атрибуция и исправления:** каждый опубликованный пример содержит ссылку на автора и канонический источник. Права остаются у владельцев. Откройте issue, чтобы изменить атрибуцию или удалить материал.

---

[![English](https://img.shields.io/badge/English-Click%20to%20View-lightgrey)](README.md) [![Español](https://img.shields.io/badge/Espa%C3%B1ol-Click%20to%20View-lightgrey)](README.es.md) [![Português](https://img.shields.io/badge/Portugu%C3%AAs-Click%20to%20View-lightgrey)](README.pt.md) [![Italiano](https://img.shields.io/badge/Italiano-Click%20to%20View-lightgrey)](README.it.md) [![Deutsch](https://img.shields.io/badge/Deutsch-Click%20to%20View-lightgrey)](README.de.md) [![Français](https://img.shields.io/badge/Fran%C3%A7ais-Click%20to%20View-lightgrey)](README.fr.md) [![العربية](https://img.shields.io/badge/%D8%A7%D9%84%D8%B9%D8%B1%D8%A8%D9%8A%D8%A9-Click%20to%20View-lightgrey)](README.ar.md) [![日本語](https://img.shields.io/badge/%E6%97%A5%E6%9C%AC%E8%AA%9E-Click%20to%20View-lightgrey)](README.ja.md) [![한국어](https://img.shields.io/badge/%ED%95%9C%EA%B5%AD%EC%96%B4-Click%20to%20View-lightgrey)](README.ko.md) [![中文](https://img.shields.io/badge/%E4%B8%AD%E6%96%87-Click%20to%20View-lightgrey)](README.zh.md)
[![Nederlands](https://img.shields.io/badge/Nederlands-Click%20to%20View-lightgrey)](README.nl.md) [![Русский](https://img.shields.io/badge/%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9-Current-brightgreen)](README.ru.md) [![Türkçe](https://img.shields.io/badge/T%C3%BCrk%C3%A7e-Click%20to%20View-lightgrey)](README.tr.md) [![Polski](https://img.shields.io/badge/Polski-Click%20to%20View-lightgrey)](README.pl.md)

---

## Создавать с Muse Video

**[Открыть рабочий процесс Muse Video в ImagineVid](https://imaginevid.io/ru/reference-video)**

Сравнивайте в этой репозитории инструкции автора с получившимся движением. Открывайте ImagineVid, когда будете готовы адаптировать грамматику кадра для нового клипа.

Популярность не является доказательством. Пост с низкой вовлечённостью, но полным промптом и полезным видео может быть ценнее вирусной демонстрации без воспроизводимой режиссуры.

| Производственная задача | Библиотека доказательств | Рабочий процесс ImagineVid |
|---------|--------------|---------------------|
| Разбор примера | Промпт, результат и источник | Генерация и сравнение |
| Поиск | Поиск текста в репозитории | Исследование по рабочим процессам |
| Генерация | - | Открыть Muse Video |
| Чтение | Markdown в стиле GitHub | Браузерное рабочее пространство продакшена |
| Рабочие процессы видео | - | Фильтры продакшена |


### Обзор по рабочему процессу

- [**Физическое движение и временная целостность**](#workflow-physical-motion-temporal-consistency) - Брифы движения для проверки веса, контакта, ускорения, баланса и непрерывности во времени.
- [**Животные, счёт и непрерывность объекта**](#workflow-animals-counting-subject-continuity) - Примеры с сохранением идентичности животных, числа объектов, анатомии и пространственных связей.
- [**Нативный звук, диалог и фоли**](#workflow-native-audio-dialogue-foley) - Промпты, где диалог, игра, атмосфера, музыка или синхронный фоли входят в режиссуру кадра.
- [**Язык камеры и иммерсивный POV**](#workflow-camera-language-immersive-pov) - Брифы кадров на основе маршрута камеры, композиции, перспективы, раскрытий и эффекта первого лица.
- [**Рекламные биты и движение продукта**](#workflow-commercial-story-beats-product-motion) - Рекламные брифы с чётким продуктом, аудиторией, последовательностью битов, движением и финальным кадром.
- [**Стилизованная и обучающая анимация**](#workflow-stylized-animation-educational-motion) - Анимация и объясняющее движение с заданной средой, правилом трансформации или учебной последовательностью.

---

## Содержание

- [Создавать с Muse Video](#создавать-с-muse-video)
- [Что такое Muse Video?](#что-такое-muse-video)
- [Официальные примеры из превью](#official-capability-cases)
- [Статус коллекции](#статус-коллекции)
- [Избранные видеопромпты](#community-featured-prompts)
- [Рабочие процессы видеопромптов](#community-prompt-cases)
- [Добавить проверенный пример](#добавить-проверенный-пример)
- [Лицензия](#лицензия)
- [Благодарности авторам](#благодарности-авторам)
- [Рост репозитории](#рост-репозитории)

---

## Что такое Muse Video?

**Muse Video** — готовящаяся модель Meta для генерации видео, показанная в июле 2026 года. Она использует общую с Muse Image базу предобучения и предназначена для создания видео с нативным звуком, точного следования подробным промптам и сохранения визуальной и временной согласованности. Meta обещает скорый доступ для авторов и в Meta AI, но не называет модель общедоступным публичным API. Компания также отмечает сохраняющиеся проблемы синхронизации звука и видео и физической точности быстрых движений.

Ниже каждый промпт оформлен как производственный бриф: действие, камера, тайминг, звук, ограничения непрерывности и подтверждающее видео хранятся вместе.

- **Начинайте с текста или кадра** — создавайте сцену по описанию или оживляйте изображение, в котором уже есть композиция
- **Описывайте наблюдаемое движение** — задавайте мизансцену, импульс, взаимодействие объектов и физическое следствие каждого действия
- **Пишите биты, а не синопсис** — используйте таймкоды или короткую последовательность действий, если важны тайминг и раскрытия
- **Создавайте звук вместе со сценой** — добавляйте диалог, атмосферу, музыку или эффекты, когда аудио служит рассказу
- **Фиксируйте точное число и идентичность** — укажите, какие персонажи, лица, объекты и пространственные связи должны сохраняться
- **Явно защищайте непрерывность** — указывайте, какие лицо, геометрия продукта, компоновка, костюм или фон не должны измениться

**Актуальные ссылки:** [Meta AI engineering overview](https://ai.meta.com/blog/introducing-muse-image-muse-video-msl/) · [Meta product announcement](https://about.fb.com/news/2026/07/introducing-muse-image-meta-ai/) · [Create videos on ImagineVid](https://imaginevid.io/ru/reference-video)

### Превратите промпт в шаблон кадра

Повторно используемые видеопромпты отделяют переменные сцены от режиссёрской логики. Заменяйте объект, место, реплику или продукт, сохраняя проверенный путь камеры, структуру битов, звуковой план и правила непрерывности.

**Шаблон:**
```
[DURATION + ASPECT RATIO]. [SUBJECT] performs [VISIBLE ACTION] in [SETTING]. Camera: [FRAMING + MOVE]. Beats: [TIMED ACTIONS]. Audio: [DIALOGUE + FOLEY + AMBIENCE]. Preserve: [IDENTITY / PRODUCT / LAYOUT]. Avoid: [FAILURE MODES].
```

Начните с одного действия и одной идеи камеры. Добавляйте тайминг, аудио и ограничения сохранения только тогда, когда они решают видимую производственную задачу; затем меняйте между генерациями по одной переменной.

---

<a id="official-capability-cases"></a>

## Официальные примеры из превью

> Воспроизводимые клипы из официального превью. Заголовки локализованы, официальные промпты сохранены на английском для проверки.

<a id="official-official-preview-clips"></a>

### Official Muse Video Preview Clips

Playable clips published by Meta and Alexandr Wang during the July 7, 2026 preview. Muse Video was still described as coming soon, not generally available.

<a id="official-case-1"></a>

#### Пример 1: Elephant at a dinner party

<img src="https://pbs.twimg.com/amplify_video_thumb/2074556635603673089/img/JlQfo2X6eAfl_d_g.jpg" height="360" alt="Elephant at a dinner party">

**Официальный исходный промпт (английский):**

```
Editorial reconstruction from the official preview: keep a full-size elephant, six guests, table setting, room geometry, reactions, and warm practical light coherent in one comic dinner-party shot.
```

---

<a id="official-case-2"></a>

#### Пример 2: Baby panda tumbling down a grassy slope

<img src="https://pbs.twimg.com/amplify_video_thumb/2074556657825202177/img/imtmutX_DBSPjify.jpg" height="360" alt="Baby panda tumbling down a grassy slope">

**Официальный исходный промпт (английский):**

```
A baby panda tumbling head over heels down a small grassy slope.
```

---

<a id="official-case-3"></a>

#### Пример 3: Four-orange juggling failure and bow

<img src="https://pbs.twimg.com/amplify_video_thumb/2074598559400247296/img/9mkUTmS0L1VPbKLA.jpg" height="360" alt="Four-orange juggling failure and bow">

**Официальный исходный промпт (английский):**

```
A man juggles three oranges, adds a fourth, drops them all, and takes a bow anyway. Warm natural morning light, gentle slow motion, one continuous approximately ten-second moment, quiet room tone, and crisp foley.
```

---

<a id="official-case-4"></a>

#### Пример 4: Exactly five ducklings climb a step

<img src="https://pbs.twimg.com/amplify_video_thumb/2074598584318611457/img/WJT3hrQ12GcGf2Ij.jpg" height="360" alt="Exactly five ducklings climb a step">

**Официальный исходный промпт (английский):**

```
A mother duck leads exactly five ducklings toward a curb; the fifth struggles, then hops up to rejoin the same group. Never add, remove, or duplicate a duckling.
```

---

<a id="official-case-5"></a>

#### Пример 5: Immersive half-frozen pond walk

<img src="https://pbs.twimg.com/amplify_video_thumb/2074599574837108737/img/xUm1Xaq31lQ8ggjj.jpg" height="360" alt="Immersive half-frozen pond walk">

**Официальный исходный промпт (английский):**

```
First-person night walk beside a half-frozen pond in falling snow, with warm lamplight on dark water, natural handheld gait, footsteps, breeze, reeds, and quiet winter ambience.
```

---

## Статус коллекции

<div align="center">

| Поле коллекции | Текущее значение |
|--------|-------|
| Проверенные примеры | **7** |
| Редакционный выбор | **5** |
| Сгенерировано | **воскресенье, 2 августа 2026 г. в 02:50:51 UTC** |

</div>

---

<a id="community-featured-prompts"></a>

## Избранные видеопромпты

> Отобрано за воспроизводимость, ясность движения и производственную пользу

<a id="prompt-1"></a>

### #1: Слон на неловком званом ужине

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### Зачем нужен этот рабочий процесс

Редакционная реконструкция официального ролика для проверки масштаба, перекрытий, реакций и комнаты.

#### Локализованный промпт

```
Фотореалистичный слон невероятным образом стоит в небольшой тепло освещённой столовой во время официального ужина. Шесть взрослых сидят за столом с жарким, свечами, цветами и бокалами. Слон заполняет дальнюю часть комнаты, не ломая стены и мебель; он тянет хобот вперёд, гости замечают его, сдержанно удивляются, а затем смеются. Один непрерывный кадр на уровне глаз с лёгким движением ручной камеры, естественными линиями взгляда, правдоподобными контактными тенями, постоянным масштабом слона, тёплыми лампами, шумом комнаты, тихим звоном приборов, движением стульев и мягким дыханием слона. Без склеек, дубликатов гостей и изменений сервировки.
```

<details>
<summary>Оригинальный промпт</summary>

```
A photorealistic elephant stands improbably inside a small, warmly lit dining room during a formal dinner party. Six adults sit around a table with roast dinner, candles, flowers, and wine glasses. The elephant fills the back of the room without breaking the walls or furniture; its trunk reaches forward as the guests notice it and react with restrained surprise, then laughter. One continuous eye-level shot with subtle handheld drift, natural eyelines, believable contact shadows, consistent elephant scale, warm practical lamps, room tone, quiet cutlery, chair movement, and a soft elephant breath. No cuts, no duplicated guests, no changing table layout.
```

</details>

#### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074556635603673089/vid/avc1/1280x720/OJwJS5B7W51JCrdH.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2074556635603673089/img/JlQfo2X6eAfl_d_g.jpg" height="420" alt="Слон на неловком званом ужине - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074556635603673089/vid/avc1/1280x720/OJwJS5B7W51JCrdH.mp4?tag=28)**
</div>

#### Доказательства

- **Автор:** [Alexandr Wang](https://x.com/alexandr_wang)
- **Канонический источник:** [Канонический источник](https://x.com/alexandr_wang/status/2074556839782416555)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="prompt-2"></a>

### #2: Панда скатывается по травяному склону

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### Зачем нужен этот рабочий процесс

Короткий официальный промпт для проверки тела, контактов, шерсти и естественной камеры.

#### Локализованный промпт

```
Детёныш панды кувырком катится вниз по небольшому травянистому склону.
```

<details>
<summary>Оригинальный промпт</summary>

```
A baby panda tumbling head over heels down a small grassy slope.
```

</details>

#### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074556657825202177/vid/avc1/1280x720/CCByPOiRRLwPhYGM.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2074556657825202177/img/imtmutX_DBSPjify.jpg" height="420" alt="Панда скатывается по травяному склону - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074556657825202177/vid/avc1/1280x720/CCByPOiRRLwPhYGM.mp4?tag=28)**
</div>

#### Доказательства

- **Автор:** [Meta Superintelligence Labs](https://x.com/AIatMeta)
- **Канонический источник:** [Канонический источник](https://x.com/alexandr_wang/status/2074556839782416555)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="prompt-3"></a>

### #3: Жонглирование четырьмя апельсинами с комической развязкой

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### Зачем нужен этот рабочий процесс

Десятисекундная арка со счётом, руками, предметами, провалом, игрой, физикой и синхронным фоли.

#### Локализованный промпт

```
Мужчина жонглирует тремя апельсинами, добавляет четвёртый, роняет все и всё равно кланяется. Тёплый естественный утренний свет, мягкое замедление. Один непрерывный момент около 10 секунд с ясным началом, поворотом и финалом. ЗВУК: тихий фон комнаты и чёткий фоли. Фотореализм, естественные свет и физика, достоверная реальная съёмка. Не мультфильм и без стилизации.
```

<details>
<summary>Оригинальный промпт</summary>

```
A man juggles three oranges, adds a fourth, drops them all, and takes a bow anyway. Warm natural morning light, gentle slow motion. A single continuous approximately 10-second moment with a clear beginning, turn, and payoff. AUDIO: quiet room tone with crisp foley. Photorealistic, natural lighting and physics, believable real-world footage. Not a cartoon, not stylized.
```

</details>

#### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074598559400247296/vid/avc1/1280x720/MT-tOwM5MF3PII-y.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2074598559400247296/img/9mkUTmS0L1VPbKLA.jpg" height="420" alt="Жонглирование четырьмя апельсинами с комической развязкой - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074598559400247296/vid/avc1/1280x720/MT-tOwM5MF3PII-y.mp4?tag=28)**
</div>

#### Доказательства

- **Автор:** [Meta AI](https://x.com/AIatMeta)
- **Канонический источник:** [Канонический источник](https://x.com/AIatMeta/status/2074600027733860758)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="prompt-6"></a>

### #4: Рекламный ролик газированного лимонада Blush Fizz

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### Зачем нужен этот рабочий процесс

Десятисекундный ролик проверяет стабильность этикетки, движение жидкости, пузырьки, декор, макрофокус и фирменный финал.

#### Локализованный промпт

```
Создайте яркий десятисекундный настольный ролик для вымышленного розового газированного лимонада «BLUSH FIZZ». В начале холодная бутылка лежит по диагонали в дроблёном льду среди лимонов, малины, мяты, розовых лепестков и капель конденсата. Надписи «BLUSH FIZZ» и «SPARKLING LEMONADE» остаются точно читаемыми. Перейдите к боковому макро: бутылка непрерывно наливает прозрачный розовый напиток в высокий стакан со льдом, лимоном, малиной и мятой. Покажите пузырьки, реалистичное преломление, мокрое стекло и мелкие брызги, не меняя продукт и декор. Завершите крупным геройским планом полного стакана и того же вымышленного логотипа справа внизу. Розово-мятная студия, яркий рассеянный свет, малая глубина резкости, плавная камера, чёткие звуки наливания и шипения. Без рук, людей, искажённой этикетки, лишнего текста и повторяющихся фруктов.
```

<details>
<summary>Оригинальный промпт</summary>

```
Create a bright ten-second tabletop commercial for a fictional sparkling pink lemonade named "BLUSH FIZZ". Open on a cold glass bottle lying diagonally in crushed ice with lemon halves, raspberries, mint leaves, pale rose petals, and condensation beads. Keep the cream label and the exact words "BLUSH FIZZ" and "SPARKLING LEMONADE" readable and unchanged. Cut to a side macro shot as the bottle tilts and pours translucent blush-pink lemonade into a clear highball glass filled with ice, lemon slices, raspberries, and mint. Show a continuous liquid stream, rising bubbles, realistic refraction, wet glass, and small splashes without changing the bottle or garnish count. Finish on a close hero shot of the full sparkling glass with a lemon wheel, raspberry, mint sprig, and the same fictional logo in the lower right. Soft pink and mint studio set, high-key diffused light, shallow depth of field, smooth product-camera motion, crisp pour and fizz sounds, no hands or people, no warped label, no extra text, no duplicate fruit.
```

</details>

#### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074600543469916160/vid/avc1/1280x720/JN6ZLW31OGfxXY1c.mp4?tag=14"><img src="https://pbs.twimg.com/amplify_video_thumb/2074600543469916160/img/pGerTyIfmrw1Cn_N.jpg" height="420" alt="Рекламный ролик газированного лимонада Blush Fizz - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074600543469916160/vid/avc1/1280x720/JN6ZLW31OGfxXY1c.mp4?tag=14)**
</div>

#### Доказательства

- **Автор:** [Ishan Misra](https://x.com/imisra_)
- **Канонический источник:** [Канонический источник](https://x.com/imisra_/status/2074600764451041536)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="prompt-7"></a>

### #5: Принцип Бернулли в покадровом потоке воздуха

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### Зачем нужен этот рабочий процесс

Учебная настольная сцена, где давление, скорость и подъёмная сила показаны понятными предметами в единой декорации.

#### Локализованный промпт

```
Создайте 12-секундное учебное стоп-моушен-видео о принципе Бернулли на ручном настольном макете. Бумажные стрелки, карточки давления, узкий канал, лёгкий шарик и две подвешенные полоски. Вид сверху: медленный поток и редкие стрелки «LOW SPEED / HIGH PRESSURE». Сузьте канал, ускорьте стрелки и замените надпись на «HIGH SPEED / LOW PRESSURE». Сбоку полоски сближаются в потоке, затем шарик висит в восходящей струе. Единые материалы, небольшие тактильные шаги, тёплая лампа, фактура бумаги, чёткий английский, фиксированные планы и мягкие звуки. Без рук, лишних формул и искажённого текста.
```

<details>
<summary>Оригинальный промпт</summary>

```
Create a 12-second stop-motion educational video explaining the Bernoulli principle on a handcrafted classroom table. Use paper arrows, labeled pressure cards, a narrow paper channel, a lightweight ball, and two suspended paper strips. Begin with a top-down view of slow airflow and wide-spaced arrows marked “LOW SPEED / HIGH PRESSURE.” Compress the channel so the arrows move faster and replace the cards with “HIGH SPEED / LOW PRESSURE.” Cut to the two paper strips lifting toward each other as air passes between them, then show the lightweight ball hovering in a rising stream. Keep every prop materially consistent and move it in small tactile increments. Warm desk lamp, paper texture, clean English labels, locked overhead and close side angles, gentle tick-and-whoosh Foley, no hands, no extra equations, no warped text.
```

</details>

#### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074600722080145408/vid/avc1/1280x720/uikPFoiwSEwIL0o1.mp4?tag=14"><img src="https://pbs.twimg.com/amplify_video_thumb/2074600722080145408/img/4YgeAwwyHuRDAR0v.jpg" height="420" alt="Принцип Бернулли в покадровом потоке воздуха - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074600722080145408/vid/avc1/1280x720/uikPFoiwSEwIL0o1.mp4?tag=14)**
</div>

#### Доказательства

- **Автор:** [Ishan Misra](https://x.com/imisra_)
- **Канонический источник:** [Канонический источник](https://x.com/imisra_/status/2074600764451041536)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="community-prompt-cases"></a>

## Рабочие процессы видеопромптов

> Сортировка по дате источника и редакционной ценности.

<a id="workflow-physical-motion-temporal-consistency"></a>

### Физическое движение и временная целостность (3)

Брифы движения для проверки веса, контакта, ускорения, баланса и непрерывности во времени.

**Избранные видеопромпты**

- [Слон на неловком званом ужине](#prompt-1)
- [Жонглирование четырьмя апельсинами с комической развязкой](#prompt-3)
- [Принцип Бернулли в покадровом потоке воздуха](#prompt-7)

<a id="workflow-animals-counting-subject-continuity"></a>

### Животные, счёт и непрерывность объекта (2)

Примеры с сохранением идентичности животных, числа объектов, анатомии и пространственных связей.

**Избранные видеопромпты**

- [Панда скатывается по травяному склону](#prompt-2)

<a id="prompt-4"></a>

#### #1: Ровно пять утят поднимаются на ступеньку

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### Зачем нужен этот рабочий процесс

Строгий промпт сохраняет пять животных, а задержка последнего создаёт препятствие и развязку.

##### Локализованный промпт

```
Утка ведёт к бордюру ровно ПЯТЬ утят; пятый и самый маленький с трудом преодолевает ступеньку, затем запрыгивает и догоняет остальных. Всего ровно пять утят, одни и те же на протяжении видео. Не добавлять, не удалять, не дублировать и не менять число; все пять видимы и стабильны от начала до конца. Уютный свет домашней лампы, правдивая любительская съёмка, немного несовершенная ручная композиция. Один непрерывный момент около 10 секунд с началом, препятствием и развязкой. Звук: мягкие шаги перепончатых лап, писк, фон комнаты, царапание о край и радостный писк после успеха последнего утёнка.
```

<details>
<summary>Оригинальный промпт</summary>

```
A mother duck leads a line of exactly FIVE ducklings toward a curb; the fifth and smallest duckling struggles at the step, then hops up to rejoin the others. There are exactly five ducklings in total — the same five throughout the entire video. Never add, remove, duplicate, or change the number of ducklings; keep all five visible and consistent from start to finish. Cozy indoor lamplight, authentic home-video look, slightly imperfect handheld framing with natural shake. One continuous approximately 10-second moment with a clear beginning, obstacle, and payoff. Audio: soft webbed footsteps, tiny chirps, room tone, a small scrape at the curb, then a bright chirp when the last duckling succeeds.
```

</details>

##### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074598584318611457/vid/avc1/1280x720/CIfMgD_fBAcwbknm.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2074598584318611457/img/WJT3hrQ12GcGf2Ij.jpg" height="420" alt="Ровно пять утят поднимаются на ступеньку - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074598584318611457/vid/avc1/1280x720/CIfMgD_fBAcwbknm.mp4?tag=28)**
</div>

##### Доказательства

- **Автор:** [Meta AI](https://x.com/AIatMeta)
- **Канонический источник:** [Канонический источник](https://x.com/AIatMeta/status/2074600027733860758)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="workflow-camera-language-immersive-pov"></a>

### Язык камеры и иммерсивный POV (1)

Брифы кадров на основе маршрута камеры, композиции, перспективы, раскрытий и эффекта первого лица.

<a id="prompt-5"></a>

#### #2: Ночная прогулка у полузамёрзшего пруда

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### Зачем нужен этот рабочий процесс

Иммерсивный POV с шагом камеры, снегом, отражениями, атмосферным звуком и непрерывным взглядом.

##### Локализованный промпт

```
Вид от первого лица во время прогулки вдоль небольшого наполовину замёрзшего пруда в заснеженном пригородном парке ночью. Берег зарос припорошённым снегом камышом и рогозом, мягко падает свежий снег. Тёплый оранжевый свет фонарей отражается в тёмной открытой воде. Спокойная атмосфера: слабое течение незамёрзшей воды, ветер в камыше, приглушённая снегом тишина и мягкие шаги. Естественная ручная камера при ходьбе, кинематографично и медитативно.
```

<details>
<summary>Оригинальный промпт</summary>

```
First-person point of view strolling along the edge of a small half-frozen pond in a snowy suburban park at night. Snow-dusted reeds and cattails line the bank; fresh snow falls gently. Warm orange lamplight reflects softly on the dark open water. Serene and peaceful, with quiet nature sounds: the faint trickle of unfrozen water, a soft breeze through the reeds, distant snow-muffled stillness, and gentle footsteps in the snow. Natural handheld walking camera, cinematic, meditative.
```

</details>

##### Видео

<div align="center">
<a href="https://video.twimg.com/amplify_video/2074599574837108737/vid/avc1/1280x720/FwxTRJjRv8yKuMqv.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2074599574837108737/img/xUm1Xaq31lQ8ggjj.jpg" height="420" alt="Ночная прогулка у полузамёрзшего пруда - Motion preview"></a>

*Нажмите на превью, чтобы открыть видео* · **[▶ Смотреть видео →](https://video.twimg.com/amplify_video/2074599574837108737/vid/avc1/1280x720/FwxTRJjRv8yKuMqv.mp4?tag=28)**
</div>

##### Доказательства

- **Автор:** [Meta AI](https://x.com/AIatMeta)
- **Канонический источник:** [Канонический источник](https://x.com/AIatMeta/status/2074600027733860758)
- **Опубликовано:** 7 июля 2026 г.
- **Язык промпта:** en

**[Создать по этой режиссуре · ImagineVid](https://imaginevid.io/ru/reference-video)**

---

<a id="workflow-commercial-story-beats-product-motion"></a>

### Рекламные биты и движение продукта (1)

Рекламные брифы с чётким продуктом, аудиторией, последовательностью битов, движением и финальным кадром.

**Избранные видеопромпты**

- [Рекламный ролик газированного лимонада Blush Fizz](#prompt-6)

## Добавить проверенный пример

Нашли пример Muse Video, который показывает реальный режиссёрский паттерн? Отправьте через GitHub Issues промпт, воспроизводимый результат, автора, источник, доказательство модели и режим ввода.

### GitHub issue

1. [**Отправить видеопромпт**](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills/issues/new?template=submit-prompt.yml)
2. Укажите полный бриф, источник, автора, доказательство модели и воспроизводимые медиа
3. Мейнтейнер проверяет происхождение, ценность видео, соответствие области и дубликаты
4. Одобренные примеры нормализуются в локальном источнике данных
5. Генератор публикует пример после прохождения всех проверок качества

**Редакционное правило:** Популярность не является доказательством. Пост с низкой вовлечённостью, но полным промптом и полезным видео может быть ценнее вирусной демонстрации без воспроизводимой режиссуры.

Перед отправкой прочитайте [CONTRIBUTING.md](docs/CONTRIBUTING.md).

---

## Лицензия

Редакционный текст и код ImagineVid распространяются по лицензии [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Сторонние промпты, личности авторов, товарные знаки, изображения и видео остаются у соответствующих владельцев и исключены из этой лицензии.

---

## Благодарности авторам

<details>
<summary>Community creators we thank (3)</summary>

[Alexandr Wang](https://x.com/alexandr_wang) · [Ishan Misra](https://x.com/imisra_) · [Meta AI](https://x.com/AIatMeta)

</details>

---

## Рост репозитории

[![GitHub stars](https://img.shields.io/github/stars/imagineVid/Awesome-muse-video-prompts-and-skills?style=for-the-badge&logo=github&label=GitHub%20Stars)](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills/stargazers)

**[Рост репозитории](https://star-history.com/#imagineVid/Awesome-muse-video-prompts-and-skills&Date)**

---

<div align="center">

**[Создавать с Muse Video](https://imaginevid.io/ru/reference-video)** •
**[Отправить проверенный пример](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills/issues/new?template=submit-prompt.yml)** •
**[Поставить звезду коллекции](https://github.com/imagineVid/Awesome-muse-video-prompts-and-skills)**

<sub>Сгенерировано из версионируемых локальных данных 2026-08-02T02:50:51.228Z</sub>

</div>
